%% This code plots the crosstream equilibria as a function of alph (for given lambda)
...
% 0.06 for Lam=0, __(check) for Lam=2.9, 0.2 for other Lam
...
maxEq = 8;
zi_Ups_IL_S = zeros(maxEq,Nalph); zi_Ups_IL_AS = zeros(maxEq,Nalph); zi_Ups_IL_U = zeros(maxEq,Nalph); 
zi_Ups_ST_S = zeros(maxEq,Nalph); zi_Ups_ST_AS = zeros(maxEq,Nalph); zi_Ups_ST_U = zeros(maxEq,Nalph); 
zi_Downs_IL_S = zeros(maxEq,Nalph); zi_Downs_IL_AS = zeros(maxEq,Nalph); zi_Downs_IL_U = zeros(maxEq,Nalph); 
zi_Downs_ST_S = zeros(maxEq,Nalph); zi_Downs_ST_AS = zeros(maxEq,Nalph); zi_Downs_ST_U = zeros(maxEq,Nalph); 
zi_Walls_IL_S = zeros(maxEq,Nalph); zi_Walls_IL_AS = zeros(maxEq,Nalph); zi_Walls_IL_U = zeros(maxEq,Nalph); 
zi_Walls_ST_S = zeros(maxEq,Nalph); zi_Walls_ST_AS = zeros(maxEq,Nalph); zi_Walls_ST_U = zeros(maxEq,Nalph); 

th_Ups_IL_S = nan(maxEq,Nalph); th_Ups_IL_AS = nan(maxEq,Nalph); th_Ups_IL_U = nan(maxEq,Nalph); 
th_Ups_ST_S = nan(maxEq,Nalph); th_Ups_ST_AS = nan(maxEq,Nalph); th_Ups_ST_U = nan(maxEq,Nalph); 
th_Downs_IL_S = nan(maxEq,Nalph); th_Downs_IL_AS = nan(maxEq,Nalph); th_Downs_IL_U = nan(maxEq,Nalph); 
th_Downs_ST_S = nan(maxEq,Nalph); th_Downs_ST_AS = nan(maxEq,Nalph); th_Downs_ST_U = nan(maxEq,Nalph); 
th_Walls_IL_S = nan(maxEq,Nalph); th_Walls_IL_AS = nan(maxEq,Nalph); th_Walls_IL_U = nan(maxEq,Nalph); 
th_Walls_ST_S = nan(maxEq,Nalph); th_Walls_ST_AS = nan(maxEq,Nalph); th_Walls_ST_U = nan(maxEq,Nalph); 
...
%     Roots3
    ...
    if (isempty(Roots3))
        ...
        plot(a*ones(size(RootsIL_AS(idxIL_AS,1))),RootsIL_AS(idxIL_AS,1),'s','MarkerFaceColor',[0 0.5 0]); hold on
        ...
        plot(a*ones(size(RootsST_AS(idxST_AS,1))),RootsST_AS(idxST_AS,1),'s','MarkerFaceColor',[0 0.5 0]); hold on
        ...
        if (sum(idxIL_AS)>0)
            zi_Ups_IL_AS(1:sum(idxIL_AS),ialph) = RootsIL_AS(idxIL_AS,1);
            th_Ups_IL_AS(1:sum(idxIL_AS),ialph) = RootsIL_AS(idxIL_AS,3);
        end
        if (sum(idxIL_U)>0)
        ...
        if (sum(idxST_AS)>0)
            zi_Ups_ST_AS(1:sum(idxST_AS),ialph) = RootsST_AS(idxST_AS,1);
            th_Ups_ST_AS(1:sum(idxST_AS),ialph) = RootsST_AS(idxST_AS,3);
        end
        if (sum(idxST_U)>0)
        ...
        count = count + sum(idxIL_S) + sum(idxIL_AS) + sum(idxIL_U) + ...
                        sum(idxST_S) + sum(idxST_U) + sum(idxST_AS);
        figure(f3a);
        plot(a*ones(size(RootsIL_S(idxIL_S,1))),RootsIL_S(idxIL_S,1),'xg'); hold on
        plot(a*ones(size(RootsIL_AS(idxIL_AS,1))),RootsIL_AS(idxIL_AS,1),'s','Color',[0 0.5 0]); hold on
        plot(a*ones(size(RootsIL_U(idxIL_U,1))),RootsIL_U(idxIL_U,1),'xr'); hold on
        title('Inline downstream');
        figure(f3b);
        plot(a*ones(size(RootsST_S(idxST_S,1))),RootsST_S(idxST_S,1),'og'); hold on
        plot(a*ones(size(RootsST_AS(idxST_AS,1))),RootsST_AS(idxST_AS,1),'s','Color',[0 0.5 0]); hold on
        ...
        if (sum(idxIL_AS)>0)
            zi_Downs_IL_AS(1:sum(idxIL_AS),ialph) = RootsIL_AS(idxIL_AS,1);
            th_Downs_IL_AS(1:sum(idxIL_AS),ialph) = RootsIL_AS(idxIL_AS,3);
        end
        if (sum(idxIL_U)>0)
        ...
        if (sum(idxST_AS)>0)
            zi_Downs_ST_AS(1:sum(idxST_AS),ialph) = RootsST_AS(idxST_AS,1);
            th_Downs_ST_AS(1:sum(idxST_AS),ialph) = RootsST_AS(idxST_AS,3);
        end
        if (sum(idxST_U)>0)
        ...
        if (sum(idxIL_AS)>0)
            zi_Walls_IL_AS(1:sum(idxIL_AS),ialph) = RootsIL_AS(idxIL_AS,1);
            th_Walls_IL_AS(1:sum(idxIL_AS),ialph) = RootsIL_AS(idxIL_AS,3);
        end
        if (sum(idxIL_U)>0)
        ...
        if (sum(idxST_AS)>0)
            zi_Walls_ST_AS(1:sum(idxST_AS),ialph) = RootsST_AS(idxST_AS,1);
            th_Walls_ST_AS(1:sum(idxST_AS),ialph) = RootsST_AS(idxST_AS,3);
        end
        if (sum(idxST_U)>0)
...
plot_lines2(zi_Ups_IL_U,alpharr,-1,0.06);
plot_lines2(zi_Ups_IL_S,alpharr,0,0.06);
plot_lines2(zi_Ups_IL_AS,alpharr,1,0.06);
...
plot_lines2(zi_Ups_ST_U,alpharr,-1,0.05);
plot_lines2(zi_Ups_ST_S,alpharr,0,0.05);
plot_lines2(zi_Ups_ST_AS,alpharr,1,0.05);
box on; ylim([-0.5 0.5]); xlabel('\alpha'); ylabel('\xi'); 
saveas(gcf,[save_fig_dir figname1 '_line_ST_Ups'],'png');

figure; set(gcf,'position',[961 415 width height]);
plot_lines2(zi_Downs_IL_U,alpharr,-1,0.08);
plot_lines2(zi_Downs_IL_S,alpharr,0,0.08);
plot_lines2(zi_Downs_IL_AS,alpharr,1,0.08);
box on; ylim([-0.5 0.5]); xlabel('\alpha'); ylabel('\xi'); 
saveas(gcf,[save_fig_dir figname1 '_line_IL_Downs'],'png');

figure; set(gcf,'position',[161 115 width height]);
plot_lines2(zi_Downs_ST_U,alpharr,-1,0.08);
plot_lines2(zi_Downs_ST_S,alpharr,0,0.08);
plot_lines2(zi_Downs_ST_AS,alpharr,1,0.08);
box on; ylim([-0.5 0.5]); xlabel('\alpha'); ylabel('\xi'); 
saveas(gcf,[save_fig_dir figname1 '_line_ST_Downs'],'png');

figure; set(gcf,'position',[561 115 width height]);
plot_lines2(zi_Walls_IL_U,alpharr,-1,0.08);
plot_lines2(zi_Walls_IL_S,alpharr,0,0.08);
plot_lines2(zi_Walls_IL_AS,alpharr,1,0.08);
box on; ylim([-0.5 0.5]); xlabel('\alpha'); ylabel('\xi'); 
saveas(gcf,[save_fig_dir figname1 '_line_IL_Walls'],'png');

figure; set(gcf,'position',[961 115 width height]);
plot_lines2(zi_Walls_ST_U,alpharr,-1,0.08);
plot_lines2(zi_Walls_ST_S,alpharr,0,0.08);
plot_lines2(zi_Walls_ST_AS,alpharr,1,0.08);
box on; ylim([-0.5 0.5]); xlabel('\alpha'); ylabel('\xi'); 
